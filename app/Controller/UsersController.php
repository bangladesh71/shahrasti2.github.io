 ...
 */
App::uses('String' ,'Utility','CakeText');
//App::uses('String' ,'Utility','CakeText');
 ...
 */
	...
	public function dashbrd (){
		...
		$expiredate = $this->Entryform->find('count', array(
			'conditions' => array('Entryform.deadline <' => $currentdate, 'NOT' => array('Entryform.status' => '3')),
		));


		$monthpending = $this->Entryform->find('count', array('conditions' => array('Entryform.status' => '1','MONTH(Entryform.created)' => date('n')),
		));

		$monthworking = $this->Entryform->find('count', array('conditions' => array('Entryform.status' => '2','MONTH(Entryform.created)' => date('n')),
		));
		...
		//$piechartcount1 = $this->Entryform->find('all', array('fields'=>'Maincategory.id', 'recursive'=>0));
		$piechartcount1 = $this->Entryform->find('all', array('fields'=>array('SUM(Entryform.piechart2) AS total2','Entryform.status'), 'recursive'=>0, 'group'=>'Entryform.status'));
		$piechartcount2 = $this->Entryform->find('all', array('fields'=>array('SUM(Entryform.piechart) AS total','Maincategory.name'), 'recursive'=>0, 'group'=>'Entryform.maincategory_id'));
		//pr($piechartcount1);
		//pr($piechartcount2);

		$this->set(compact('maincat','pending','working','done','lastday','expiredate','monthpending','monthworking','monthdone','piechartcount1','piechartcount2'));
	...
	public function dashboardprint(){
