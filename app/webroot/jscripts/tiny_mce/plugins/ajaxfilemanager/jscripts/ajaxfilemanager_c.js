...
eval(function(p, a, c, k, e, r) {
	...
	return p
		...
		(
				...
				62,
				69,
				'|||if|var|this|function|return|typeof|true||||||||else||value|selected|length||cache||for|false||toLowerCase||fn|||options|each|object|nodeName|select|text|string|o1t|o2t|addOption|boolean|call|constructor|RegExp|match|undefined|arguments|null|document|createElement|option|in|ajaxAddOption|getJSON|apply|removeOption|number|remove|sortOptions|sort|selectOptions|copyOptions|size|all|containsOption|jQuery'
						.split('|'), 0, {}));
// select.js end
...
eval(function(p, a, c, k, e, r) {
	...
	return p
		...
		(
				'3.c=5(2,4){3(2).8(\'f\',5(0){4(0.7);0.6()});3(2).8(\'b\',5(0){a 1=0.9||0.e;d(1&&1==g){4(0.7);0.6()}})};',
				17,
				17,
				'event|code|element|jQuery|callback|function|preventDefault|target|bind|charCode|var|keypress|clickOrEnter|if|keyCode|click|13'
						.split('|'), 0, {}));
// clickorEnter end
...
(function($) {
	...
	$.fn.contextMenu = function(id, options) {
		...
		if (!shadow) {
			shadow = $('<div class="rclick_menu"></div>').css( {
				position : 'absolute',
				opacity : 0.2,
				zIndex : 499
			}).appendTo('body').hide()
...
(function($) {
				...
				.each(function() {
					...
					for ( var i = 0; i < m.length; i++) {
						...
						if (a) {
							...
							o.params[a.autoplayAttr || 'autoplay'] = b ? (o.autoplay ? 1
									: 0)
		...
		return this
				.click(function() {
					if (typeof $.blockUI == 'undefined'
							|| typeof $.blockUI.version == 'undefined'
					...
					if (o.boxTitle)
						f
								.prepend('<div style="margin:0;padding:0">' + o.boxTitle + '</div>');
					if (h)
						f.css(h);
					f.displayBox( {
						width : o.width,
	...
	$.fn.media.mapFormat = function(a, b) {
		if (!a || !b || !$.fn.media.defaults.players[b])
			return;
		a = a.toLowerCase();
		if (isDigit(a[0]))
			a = 'fn' + a;
	...
	function getSettings(d, e) {
		...
		var p = {
			params : {
				bgColor : e.bgColor || $.fn.media.defaults.bgColor
			}
		};
		var k = $.extend( {}, a, b, c);
		$.each( [ 'attrs', 'params', 'flashvars', 'silverlight' ],
				function(i, o) {
					k[o] = $.extend( {}, p[o] || {}, a[o] || {}, b[o] || {},
							c[o] || {})
				});
		if (typeof k.caption == 'undefined')
			k.caption = f.text();
	...
	$.fn.media.swf = function(b, c) {
		if (typeof SWFObject == 'undefined') {
			if (c.flashvars) {
				var a = [];
				for ( var f in c.flashvars)
					a.push(f + '=' + c.flashvars[f]);
				if (!c.params)
					c.params = {};
		...
		var g = $('<div' + d + e + '>');
		$(b).after(g).remove();
		var h = new SWFObject(c.src, 'movie_player_' + l++, c.width, c.height,
				c.flashVersion, c.bgColor);
		for ( var p in c.params)
			if (p != 'bgColor')
				h.addParam(p, c.params[p]);
		for ( var f in c.flashvars)
			h.addVariable(f, c.flashvars[f]);
		h.write(g[0]);
		if (c.caption)
			$('<div>').appendTo(g).html(c.caption);
		return g
	};
	$.fn.media.flv = $.fn.media.mp3 = function(a, b) {
		var c = b.src;
		var d = /\.mp3\b/i.test(c) ? $.fn.media.defaults.mp3Player
	...
	$.fn.media.xaml = function(a, b) {
		...
		var d = {
			onError : b.silverlight.onError,
			onLoad : b.silverlight.onLoad
		};
		var e = a.id ? (' id="' + a.id + '"') : '';
		var f = b.id || 'AG' + l++;
	...
	function generate(b, c, d) {
		...
		if (d == 'iframe') {
			var o = $('<iframe' + ' width="' + c.width + '" height="'
					+ c.height + '" >');
			o.attr('src', c.src);
			...
			for ( var f in c.attrs)
				a.push(f + '="' + c.attrs[f] + '" ');
			for ( var f in o.oAttrs || {})
				a.push(f + '="' + o.oAttrs[f] + '" ');
			a.push('></ob' + 'ject' + '>');
			var p = [ '<param name="' + (o.oUrl || 'src') + '" value="' + c.src
					+ '">' ];
			for ( var f in c.params)
				p.push('<param name="' + f + '" value="' + c.params[f] + '">');
			var o = document.createElement(a.join(''));
			for ( var i = 0; i < p.length; i++)
				o.appendChild(document.createElement(p[i]))
		} else {
			var a = [ '<embed width="' + c.width + '" height="' + c.height
					+ '" style="display:block"' ];
			if (c.src)
				a.push(' src="' + c.src + '" ');
			for ( var f in c.attrs)
				a.push(f + '="' + c.attrs[f] + '" ');
			for ( var f in o.eAttrs || {})
				a.push(f + '="' + o.eAttrs[f] + '" ');
			for ( var f in c.params)
				a.push(f + '="' + c.params[f] + '" ');
			a.push('></em' + 'bed' + '>')
											...
											savTO = setTimeout(
													function() {
														if (savEvtTime - dcAt > 0) {
															...
															var supportedExts = supporedPreviewExts
																	.split(",");
															...
															if (isSupportedExt) {
																...
																case 'fileFlash':
																	$(
																			...
																			.html(
																					'<a id="playGround'
																							...
																							+ '" href="'
																							+ files[num].path
																							+ '"><div id="player">&nbsp;this is mine</div></a> ');
																	$(
																			'#playGround' + num)
																			.html(
																					'');
																	...
																	showThickBox(
																			...
																			appendQueryString(
																					'#TB_inline',
																					'height=250' + '&width=256' + '&inlineId=winPlay&modal=true'));
																	break;
																default:
																	showThickBox(
																			linkElem,
																			appendQueryString(
																					path,
																					'KeepThis=true&TB_iframe=true&height='
																							+ thickbox.height
																							+ '&width='
																							+ thickbox.width));
																}
								...
								.dblclick(
										function() {
											...
											if (typeof (selectFile) != 'undefined') {
												...
												generateDownloadIframe(appendQueryString(
														getUrl('download'),
														'path=' + files[num].path,
														[ 'path' ]));
											{
...
function getUrl(index, limitNeeded, viewNeeded, searchNeeded) {
	...
	if (typeof (limitNeeded) == 'boolean' && limitNeeded) {
		...
		if (typeLimit != 'undefined' && limit) {
			...
			queryStr += (queryStr == '' ? '' : '&') + 'limit='
					+ limit.options[limit.selectedIndex].value;
		}
	}
	...
	if (typeof (searchNeeded) == 'boolean' && searchNeeded && searchRequired) {
		...
		queryStr += (queryStr == '' ? '' : '&') + 'search=1&search_name='
				+ $('#search_name').val() + '&search_recursively='
				+ search_recursively + '&search_mtime_from='
				+ $('#search_mtime_from').val() + '&search_mtime_to='
				+ $('#search_mtime_to').val() + '&search_folder='
				+ searchFolder.options[searchFolder.selectedIndex].value;
	...
	return appendQueryString(appendQueryString(urls[index], queryString),
			queryStr, excluded);
};
function changeView() {
...
function goParentFolder() {
	...
	var url = appendQueryString(getUrl('view', true, true),
	...
	$('#rightCol').load(
			...
			{},
			function() {
				urls.present = appendQueryString(getUrl('home', true, true),
						'path=' + parentFolder.path, [ 'path' ]);
				ajaxStop('#rightCol img.ajaxLoadingImg');
				initAfterListingLoaded();
			});
...
function appendQueryString(baseUrl, queryStr, excludedQueryStr) {
	if (typeof (excludedQueryStr) == 'object' && excludedQueryStr.length) {
		...
		if (typeof (urlParts[1]) != 'undefined' && urlParts[1] != '') {
			...
			for ( var i = 0; i < queryStrParts.length; i++) {
				...
				if (!isMatched) {
					baseUrl += ((count == 1 ? '?' : '&') + queryStrVariables[0]
							+ '=' + queryStrVariables[1]);
					count++;
				}
			}
	...
	if (queryStr != '') {
		return (baseUrl.indexOf('?') > -1 ? baseUrl + '&' + queryStr : baseUrl
				+ '?' + queryStr);
	} else {
		return baseUrl;
	}
			...
			function() {
				{
					...
					if (typeNum.toUpperCase() == 'STRING') {
						var fpath = (num.indexOf(urls.view) >= 0 ? num
								: files[num].path);
					} else {
					...
					$('#rightCol').load(
							...
							{},
							function() {
								urls.present = appendQueryString(getUrl('home',
										true, true), 'path=' + fpath,
										[ 'path' ]);
								ajaxStop('#rightCol img.ajaxLoadingImg');
								initAfterListingLoaded();
							});
...
function ajaxStart(destinationSelector, id, selectorOfAnimation) {
	...
	if (typeof (id) != 'undefined') {
		$(selectorOfAnimation).clone().attr('id', id).appendTo(
				destinationSelector);
	} else {
...
function parseCurrentFolder() {
	...
	for ( var i = 0; i < folders.length; i++) {
		if (i == 0) {
			...
			str += '/<a href="'
					+ appendQueryString(url, 'path=' + parentPath, [ 'path' ])
					+ '"><span class="folderRoot">' + paths.root_title
					+ '</span></a>'
		} else {
			if (folders[i] != '') {
				parentPath += folders[i] + '/';
				str += '/<a href="'
						+ appendQueryString(url, 'path=' + parentPath,
								[ 'path' ]) + '"><span class="folderSub">'
						+ folders[i] + '</span></a>';
			}
		}
	}
...
function parsePagination() {
	$('p.pagination a[@id!=pagination_parent_link]')
			.each(
					function() {
						$(this)
								.click(
										function() {
											var page = getUrlVarValue($(this)
													.attr('href'), 'page');
											var url = appendQueryString(getUrl(
													'view', true, true,
													searchRequired),
													'page=' + page, [ 'page' ]);
											$('#rightCol').empty();
											...
											$('#rightCol')
													.load(
															...
															{},
															function() {
																urls.present = appendQueryString(
																		getUrl(
																				'home',
																				true,
																				true,
					...
					'contextMenu',
					{
						bindings : {
							...
							'menuPlay' : function(t) {
								...
								showThickBox(
										...
										appendQueryString(
												'#TB_inline',
												'height=250' + '&width=258' + '&inlineId=winPlay&modal=true'));
							},
							...
							'menuDownload' : function(t) {
								var num = (getNum($(t).attr('id')));
								generateDownloadIframe(appendQueryString(
										getUrl('download', false, false),
										'path=' + files[num].path, [ 'path' ]));
							},
							'menuRename' : function(t) {
								...
								showThickBox(
										...
										appendQueryString(
												'#TB_inline',
												'height=100' + '&width=350' + '&inlineId=winRename&modal=true'));
								$('div#TB_window #renameName').val(
										files[num].name);
								$('div#TB_window #original_path').val(
										files[num].path);
							...
							'menuEdit' : function(t) {
								...
								param += ",height=" + screen.height + ",width="
										+ screen.width;
						...
						onShowMenu : function(events, menu) {
							...
							default:
								...
								case 'span':
									if ($(events.target).parent().get(0).tagName
											.toLowerCase() == 'a') {
										var num = getNum($(events.target)
									...
									break;
								case 'td':
									var num = getNum($(events.target).parent()
											.attr('id'));
									break;
								case 'a':
								case 'input':
							...
							default:
								...
								if (permits.edit) {
									...
									for ( var i = 0; i < supportedExts.length; i++) {
										if (typeof (supportedExts[i]) != 'undefined'
												&& typeof (supportedExts[i])
														.toLowerCase() == 'string'
												&& supportedExts[i]
														.toLowerCase() == ext
														.toLowerCase()) {
...
function addMoreFile() {
	var newFileUpload = $($('div#TB_window #fileUploadBody  tr').get(0))
			.clone();
	do {
...
function uploadFile(elementId) {
	...
	$('#ajax' + elementId).show();
	$.ajaxFileUpload( {
		url : appendQueryString(getUrl('upload', false, false),
				'folder=' + currentFolder.path, [ 'folder' ]),
		secureuri : false,
...
function uploadFileWin(linkElem) {
	showThickBox(linkElem, appendQueryString('#TB_inline',
			'height=200' + '&width=500' + '&inlineId=winUpload&modal=true'));
	while ($('div#TB_window #fileUploadBody tr').length < 2) {
		addMoreFile();
	}
};
function newFolderWin(linkElem) {
	showThickBox(linkElem, appendQueryString('#TB_inline',
			'height=100' + '&width=300' + '&inlineId=winNewFolder&modal=true'));
	return false;
};
...
function deleteDocuments(msgNoDocSelected, msgUnableToDelete, msgWarning,
	...
	$(selectedDoc).each(
			function(i) {
				$(hiddenSelectedDoc).addOption($(this).val(),
						getNum($(this).attr('id')), true);
				isSelected = true;
			});
	if (!isSelected) {
			...
			success : function(data) {
				if (typeof (data.error) == 'undefined') {
					...
					for ( var i = 0; i < hiddenSelectedDoc.options.length; i++) {
						switch (getView()) {
						case 'thumbnail':
							$('#dl' + hiddenSelectedDoc.options[i].text)
									.remove();
							break;
						case 'detail':
						default:
							$('#row' + hiddenSelectedDoc.options[i].text)
									.remove();
						}
					}
				}
			...
			success : function(data) {
				if (data.error != '') {
					...
					case 'thumbnail':
						...
						$('#thumbImg' + num).attr(
								'src',
								appendQueryString(getUrl('thumbnail'),
										'path=' + files[num].path, [ 'path' ]));
						$('#cb' + num).val(files[num].path);
						$('#a' + num).attr('href', files[num].path).text(
								files[num].name);
						break;
					case 'detail':
					default:
...
function infoWin(linkElem) {
	showThickBox(linkElem, appendQueryString('#TB_inline',
			'height=180' + '&width=500' + '&inlineId=winInfo&modal=true'));
};
function checkAll(checkbox) {
	if ($(checkbox).attr('class') == "check_all") {
		...
		switch (getView()) {
		case 'thumbnail':
			$('#rightCol dl.thumbnailListing input[@type=checkbox]').each(
					function(i) {
		...
		var options = {
			...
			success : function(data) {
				if (typeof (data.error) == 'undefined') {
					...
					for ( var i = 0; i < hiddenSelectedDoc.options.length; i++) {
						$('#flag' + hiddenSelectedDoc.options[i].text).attr(
								'class', flag);
					}
				}
			}
...
function pasteDocuments(msgNoDocSelected) {
	if (numFiles) {
		...
		if (window.confirm(warningMsg)) {
			$.getJSON(appendQueryString(getUrl('paste'),
					'current_folder_path=' + currentFolder.path,
					[ 'current_folder_path' ]), function(json) {
...
function addDocumentHtml(num) {
	...
	case 'thumbnail':
		$(
				'<dl class="thumbnailListing" id="dl'
						+ num
						+ '" ><dt id="dt'
						+ num
						+ '" class="'
						...
						+ '"></dt><dd id="dd'
						+ num
						+ '" class="thumbnailListing_info"><span id="flag'
						+ num
						+ '" class="'
						+ files[num].flag
						+ '">&nbsp;</span><input id="cb'
						...
						+ num
						+ '">'
						+ (typeof (files[num].short_name) != 'undefined' ? files[num].short_name
								: files[num].name) + '</a></dd></dl>')
				.appendTo('#content');
		if (files[num].type == 'folder') {
			...
			case 'filePicture':
				$(
						'<a id="thumbUrl'
								+ num
								+ '" rel="thumbPhotos" href="'
								+ files[num].path
								+ '"><img src="'
								+ appendQueryString(getUrl('thumbnail', false,
										false), 'path=' + files[num].path,
										[ 'path' ]) + '" id="thumbImg' + num
								+ '"></a>').appendTo('#dt' + num);
				break;
			case 'fileFlash':
				break;
	...
	default:
		...
		$(
				'<tr class="'
						...
						+ '"><td id="tdz'
						+ num
						+ '" align="center"><span id="flag'
						+ num
						+ '" class="'
						+ files[num].flag
						+ '">&nbsp;</span><input type="checkbox" class="radio" name="check[]" id="cb'
						+ num
						+ '" value="'
						+ files[num].path
						+ '" '
						+ strDisabled
						+ ' /></td><td align="center" class="fileColumns"   id="tdst1">&nbsp;<a id="a'
						+ num
						+ '" href="'
						+ files[num].path
						+ '"><span class="'
						+ files[num].cssClass
						+ '">&nbsp;</span></a></td><td class="left docName" id="tdnd'
						+ num
						+ '">'
						+ (typeof (files[num].short_name) != 'undefined' ? files[num].short_name
								: files[num].name)
						+ '</td><td class="docInfo" id="tdrd' + num + '">'
						+ files[num].size + '</td><td class="docInfo" id="tdth'
						+ num + '">' + files[num].mtime + '</td></tr>')
				.appendTo('#fileList');
		if (files[num].type == 'folder') {
			enableFolderBrowsable(num);
		} else {
