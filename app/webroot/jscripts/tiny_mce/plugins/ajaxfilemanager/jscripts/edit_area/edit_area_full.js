...
EAL.prototype.init = function(settings) {
	...
	if (this.error)
		return;
	if (eAs[settings["id"]])
		eAL.delete_instance(settings["id"]);
	...
	if (settings["begin_toolbar"].length > 0)
		settings["toolbar"] = settings["begin_toolbar"] + ","
				+ settings["toolbar"];
	if (settings["end_toolbar"].length > 0)
		settings["toolbar"] = settings["toolbar"] + ","
				+ settings["end_toolbar"];
	...
	if (settings["syntax"].length > 0) {
		...
		this.load_script(this.baseURL + "reg_syntax/" + settings["syntax"]
				+ ".js");
	}
	eAs[settings["id"]] = {
		"settings" : settings
...
EAL.prototype.delete_instance = function(id) {
	...
	if (window.frames["frame_" + id] && eAs[id]["displayed"])
...
EAL.prototype.start = function(id) {
	...
	if (!eAL.lang[eAs[id]["settings"]["language"]]
			|| (eAs[id]["settings"]["syntax"].length > 0 && !eAL.load_syntax[eAs[id]["settings"]["syntax"]])) {
		setTimeout("eAL.start('" + id + "');", 50);
		return;
	}
	if (eAs[id]["settings"]["syntax"].length > 0)
		eAL.init_syntax_regexp();
	if (!document.getElementById("EditAreaArroundInfos_" + id)
		...
		if (eAs[id]["settings"]["debug"])
			html += "<textarea id='edit_area_debug_"
					+ id
					+ "' style='z-index: 20;width: 100%;height: 120px;overflow: auto;border: solid black 1px;'></textarea><br />";
	...
	for ( var i = 0; i < area["settings"]["tab_toolbar"].length; i++) {
		html_toolbar_content += this.get_control_html(
	...
	if (!this.iframe_script) {
		...
		for ( var i = 0; i < this.sub_scripts_to_load.length; i++)
			this.iframe_script += '<script language="javascript" type="text/javascript" src="'
					+ this.baseURL
					+ this.sub_scripts_to_load[i]
					+ '.js"></script>';
	}
	for ( var i = 0; i < area["settings"]["plugins"].length; i++) {
		if (!eAL.all_plugins_loaded)
			this.iframe_script += '<script language="javascript" type="text/javascript" src="'
					+ this.baseURL
					+ 'plugins/'
		...
		this.iframe_script += '<script language="javascript" type="text/javascript" src="'
				...
				+ 'plugins/'
				+ area["settings"]["plugins"][i]
				+ '/langs/' + area["settings"]["language"] + '.js"></script>';
	}
	if (!this.iframe_css) {
		this.iframe_css = "<link href='" + this.baseURL
	...
	template = this.translate(template, area["settings"]["language"],
			"template");
...
EAL.prototype.set_editarea_size_from_textarea = function(id, frame) {
	var elem = document.getElementById(id);
	var width = Math.max(eAs[id]["settings"]["min_width"], elem.offsetWidth)
			+ "px";
	var height = Math.max(eAs[id]["settings"]["min_height"], elem.offsetHeight)
			+ "px";
...
EAL.prototype.set_base_url = function() {
	if (!this.baseURL) {
		...
		for ( var i = 0; i < elements.length; i++) {
			if (elements[i].src
					&& elements[i].src.match(/edit_area_[^\\\/]*$/i)) {
				var src = elements[i].src;
				src = src.substring(0, src.lastIndexOf('/'));
				this.baseURL = src;
				this.file_name = elements[i].src.substr(elements[i].src
						.lastIndexOf("/") + 1);
				break;
			}
		}
...
EAL.prototype.get_button_html = function(id, img, exec, baseURL) {
	...
	html = '<a href="javascript:' + cmd + '" onclick="' + cmd
			+ ';return false;" onmousedown="return false;" target="_self">';
	html += '<img id="'
			+ id
			+ '" src="'
			+ baseURL
...
EAL.prototype.get_control_html = function(button_name, lang) {
	...
	case "select_font":
		html = "<select id='area_font_size' onchange='javascript:editArea.execCommand(\"change_font_size\")'>"
				...
				+ "			<option value='14'>14 pt</option>" + "		</select>";
		return html;
	}
...
EAL.prototype.submit = function(e) {
	...
	for ( var i in eAs) {
		...
		if (is_child) {
			if (window.frames["frame_" + i] && eAs[i]["displayed"] == true)
...
EAL.prototype.setSelectionRange = function(id, new_start, new_end) {
	if (window.frames["frame_" + id] && eAs[id]["displayed"] == true) {
		window.frames["frame_" + id].editArea.area_select(new_start, new_end
				- new_start);
		if (!this.nav['isIE']) {
			window.frames["frame_" + id].editArea.check_line_selection(false);
...
EAL.prototype.setSelectedText = function(id, new_val) {
	...
	if (window.frames["frame_" + id] && eAs[id]["displayed"] == true) {
		var scrollTop = window.frames["frame_" + id].document
				.getElementById("result").scrollTop;
		var scrollLeft = window.frames["frame_" + id].document
...
function start_move_element(e, id, frame) {
	...
	mouse_x = getMouseX(e);
	mouse_y = getMouseY(e);
	move_current_element.start_pos_x = mouse_x
			- (move_current_element.style.left.replace("px", "") || calculeOffsetLeft(move_current_element));
	move_current_element.start_pos_y = mouse_y
			- (move_current_element.style.top.replace("px", "") || calculeOffsetTop(move_current_element));
	return false;
};
...
function move_element(e) {
	...
	var max_left = move_current_element.frame.document.body.offsetWidth
			- move_current_element.offsetWidth;
	max_top = move_current_element.frame.document.body.offsetHeight
			- move_current_element.offsetHeight;
...
function get_IE_selection(textarea) {
	if (textarea && textarea.focused) {
		...
		if (stored_range.parentElement() == textarea) {
			...
			var relative_top = range.offsetTop - calculeOffsetTop(textarea)
					+ scrollTop;
			var line_start = Math
					.round((relative_top / textarea.ea_line_height) + 1);
			var line_nb = Math.round(range.boundingHeight
					/ textarea.ea_line_height);
			...
			tab = textarea.value.substr(0, range_start + range.text.length)
					.split("\n");
			range_end += (line_start + line_nb - 1 - tab.length) * 2;
			textarea.selectionEnd = range_end;
		}
	}
	setTimeout("get_IE_selection(document.getElementById('" + textarea.id
			+ "'));", 50);
};
...
function set_IE_selection(textarea) {
	...
	range.moveEnd('character', textarea.selectionEnd - nbLineEnd
			- (textarea.selectionStart - nbLineStart));
	range.select();
};
...
EAL.prototype.end_resize_area = function(e) {
	...
	var div = document.getElementById("edit_area_resize");
	var textarea = eAs[eAL.resize["id"]]["textarea"];
	var width = Math.max(eAs[eAL.resize["id"]]["settings"]["min_width"],
			div.offsetWidth - 4);
	var height = Math.max(eAs[eAL.resize["id"]]["settings"]["min_height"],
			div.offsetHeight - 4);
	if (eAL.nav['isIE'] == 6) {
		width -= 2;
		height -= 2;
...
EAL.prototype.resize_area = function(e) {
	...
	if (allow == "both" || allow == "y") {
		...
		document.getElementById("edit_area_resize").style.height = new_height
				+ "px";
	}
	if (allow == "both" || allow == "x") {
		new_x = getMouseX(e);
		var new_width = Math.max(20, new_x - eAL.resize["start_left"]);
		document.getElementById("edit_area_resize").style.width = new_width
				+ "px";
	}
	return false;
};
