(function() {
	var d = tinymce
			.explode("id,name,width,height,style,align,class,hspace,vspace,bgcolor,type"), h = tinymce
			.makeMap(d.join(",")), b = tinymce.html.Node, f, a, g = tinymce.util.JSON, e;
	f = [
			[
					"Flash",
			...
			.create(
					"tinymce.plugins.MediaPlugin",
					{
						init : function(n, j) {
							...
							function o(i) {
								return i && i.nodeName === "IMG"
										&& n.dom.hasClass(i, "mceItemMedia")
							}
							r.editor = n;
							r.url = j;
							a = "";
							...
							n.addCommand("mceMedia", function() {
								...
								if (o(i)) {
									...
									if (s) {
										...
										s.type = r.getType(i.className).name
												.toLowerCase()
									}
								}
								if (!s) {
								...
								n.windowManager.open( {
									file : j + "/media.htm",
									width : 430 + parseInt(n.getLang(
											"media.delta_width", 0)),
									height : 500 + parseInt(n.getLang(
											"media.delta_height", 0)),
									inline : 1
								}, {
									plugin_url : j,
									data : s
						...
						convertUrl : function(k, n) {
							var j = this, m = j.editor, l = m.settings, o = l.url_converter, i = l.url_converter_scope
									|| j;
							if (!k) {
								return k
							}
						...
						dataToImg : function(m, k) {
							...
							n = r.editor.dom.create("img", {
								...
								"class" : "mceItemMedia mceItem"
										+ r.getType(m.type).name,
								"data-mce-json" : g.serialize(m, "'")
							});
							n.width = m.width
									|| (m.type == "audio" ? "300" : "320");
							n.height = m.height
									|| (m.type == "audio" ? "32" : "240");
							return n
						},
						dataToHtml : function(i, j) {
							return this.editor.serializer.serialize(this
									.dataToImg(i, j), {
								forced_root_block : "",
								force_absolute : j
							})
						...
						htmlToData : function(k) {
							...
							if (i) {
								...
								l.type = this.getType(i.attr("class")).name
										.toLowerCase();
						...
						imgToObject : function(z, o) {
							...
							function r(i, n) {
								var M, L, N, K, J;
								J = p
										.getParam("flash_video_player_url", u
												.convertUrl(u.url
														+ "/moxieplayer.swf"));
								if (J) {
									M = p.documentBaseURI;
							...
							if (!B) {
								B = z.attr("style");
								if (B) {
									B = p.dom.serializeStyle(p.dom.parseStyle(
											B, "img"))
								}
							}
							if (q.name === "Iframe") {
							...
							if (this.editor.settings.media_use_script) {
								x = new b("script", 1).attr("type",
										"text/javascript");
								y = new b("#text", 3);
								y.value = "write"
										+ q.name
										+ "("
										+ g.serialize(tinymce.extend(G.params,
												{
													width : z.attr("width"),
							...
							if (G.params.src) {
								...
								if (o && o.force_absolute) {
									G.params.src = p.documentBaseURI
											.toAbsolute(G.params.src)
								}
						...
						objectToImg : function(C) {
							...
							if (N === "video" || N == "audio") {
								...
								if (z.poster) {
									z.poster = v.call(n, z.poster, "poster",
											C.name)
								}
							}
							if (C.name === "object") {
							...
							if (K.params.src) {
								K.params.src = v.call(n, K.params.src, "src",
										"object")
							}
							if (F) {
								if (C.name === "video") {
